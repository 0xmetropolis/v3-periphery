...
contract NFTDescriptor {
    ...
    function constructTokenURI(ConstructTokenURIParams memory params) public pure returns (string memory) {
        string memory name = _generateName(params, _feeToPercentString(params.fee));
        string memory descriptionPartOne =
            _generateDescriptionPartOne(
                _escapeQuotes(params.quoteTokenSymbol),
                _escapeQuotes(params.baseTokenSymbol),
                _addressToString(params.poolAddress)
            );
        string memory descriptionPartTwo =
            _generateDescriptionPartTwo(
                params.tokenId.toString(),
                _escapeQuotes(params.baseTokenSymbol),
                _addressToString(params.quoteTokenAddress),
                _addressToString(params.baseTokenAddress),
                _feeToPercentString(params.fee)
            );
        string memory image = Base64.encode(bytes(_generateSVGImage(params)));
    ...
    function _generateDescriptionPartOne(
    ...
    function _generateDescriptionPartTwo(
    ...
    function _generateName(ConstructTokenURIParams memory params, string memory feeTier)
        private
        pure
    ...
    function _tickToDecimalString(
        ...
        if (tick == (TickMath.MIN_TICK / tickSpacing) * tickSpacing) {
            ...
            return _fixedPointToDecimalString(sqrtRatioX96, baseTokenDecimals, quoteTokenDecimals);
        }
    }
    ...
    function _fixedPointToDecimalString(
        uint160 sqrtRatioX96,
        uint8 baseTokenDecimals,
        uint8 quoteTokenDecimals
    ) internal pure returns (string memory) {
    ...
    function _generateSVGImage(ConstructTokenURIParams memory params) internal pure returns (string memory svg) {
        NFTSVG.SVGParams memory svgParams =
            NFTSVG.SVGParams({
                ...
                color0: _tokenToColorHex(uint256(params.quoteTokenAddress), 136),
                color1: _tokenToColorHex(uint256(params.baseTokenAddress), 136),
                color2: _tokenToColorHex(uint256(params.quoteTokenAddress), 0),
                color3: _tokenToColorHex(uint256(params.baseTokenAddress), 0),
                x1: scale(_getCircleCoord(uint256(params.quoteTokenAddress), 16, params.tokenId), 0, 255, 16, 274),
                y1: scale(_getCircleCoord(uint256(params.baseTokenAddress), 16, params.tokenId), 0, 255, 100, 484),
                x2: scale(_getCircleCoord(uint256(params.quoteTokenAddress), 32, params.tokenId), 0, 255, 16, 274),
                y2: scale(_getCircleCoord(uint256(params.baseTokenAddress), 32, params.tokenId), 0, 255, 100, 484),
                x3: scale(_getCircleCoord(uint256(params.quoteTokenAddress), 48, params.tokenId), 0, 255, 16, 274),
                y3: scale(_getCircleCoord(uint256(params.baseTokenAddress), 48, params.tokenId), 0, 255, 100, 484)
            });
    ...
    function _tokenToColorHex(uint256 token, uint256 offset) internal pure returns (string memory str) {
        return string((token >> offset).toHexStringNoPrefix(3));
    }

    function _getCircleCoord(
        uint256 tokenAddress,
        uint256 offset,
        uint256 tokenId
    ) internal pure returns (uint256) {
        return (_sliceTokenHex(tokenAddress, offset) * tokenId) % 255;
    }

    function _sliceTokenHex(uint256 token, uint256 offset) internal pure returns (uint256) {
