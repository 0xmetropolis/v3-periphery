...
contract NFTDescriptor {
    ...
    function constructTokenURI(ConstructTokenURIParams memory params) external pure virtual returns (string memory) {
        string memory name = _generateName(params, _feeToPercentString(params.fee));
        string memory descriptionPartOne = _generateDescriptionPartOne(
            _escapeQuotes(params.quoteTokenSymbol),
            _escapeQuotes(params.baseTokenSymbol),
            _addressToString(params.poolAddress)
        );
        string memory descriptionPartTwo = _generateDescriptionPartTwo(
    ...
    function _fixedPointToDecimalString(uint160 sqrtRatioX96, uint8 baseTokenDecimals, uint8 quoteTokenDecimals)
        internal
        pure
        returns (string memory)
    {
